...
;; env-compress and env-expand were generated by envcomp.scm.
;;
...
(define (env-cmp s)
  (subst ";" "!A" "\\" "!B" "," "!C" "`" "!D" "'" "!E" "<" "!F" ">" "!G" "["
         ...
         "&" "!P" "(" "!Q" ")" "!R" "+" "!S" "_" "!T" "!0" ";" "10" "\\" "!\\"
         "," "!1\\" "`" "!1111" "'" "!11\\" "<" "!111\\" ">" "11:P" "["
         "!=!1:EDefn" "]" "]1;~%;" "|" "ml.special-" "@" "|p;" "{" "{1 " "}"
         ";![0," "#" ",!1[2`" "\"" ",!1[0`" "&" "`':P2<" "(" "]1;:;p;" ")"
         "<'1:P2>" "+" ",!1[1`" "_" s))

(define (env-exp s)
  (subst "_" ",!1[1`" "+" "<'1:P2>" ")" "]1;:;p;" "(" "`':P2<" "&" ",!1[0`"
         "\"" ",!1[2`" "#" ";![0," "}" "{1 " "{" "|p;" "@" "ml.special-" "|"
         "]1;~%;" "]" "!=!1:EDefn" "[" "11:P" ">" "!111\\" "<" "!11\\" "'"
         "!1111" "`" "!1\\" "," "!\\" "\\" "10" ";" "!0" "!T" "_" "!S" "+" "!R"
         ")" "!Q" "(" "!P" "&" "!O" "\"" "!N" "#" "!M" "}" "!L" "{" "!K" "@"
         "!J" "|" "!I" "]" "!H" "[" "!G" ">" "!F" "<" "!E" "'" "!D" "`" "!C"
         "," "!B" "\\" "!A" ";" s))
...
;; Replace each import marker with itself AND all the public symbols from
;; the module it references.
;;
(define (expand-import-markers env)
  (foreach b env
           (if (filter (concat [ImportMarkerKey] "!=%") b)
               (get-module-env (case (hash-value b)
                                 ((EMarker file) (promote file)))
                               nil)
               b)))
...
;;        setting each entry's `scope` to 'iFILENAME', and an
;;        import marker.
;; D-NAME = name of the module containing the exports (demoted)
;;
(define (env-import env all d-name)
  (if all
      ;; Reconstruct final env: expand import markers
      (expand-import-markers env)
      ;; Get only public symbols
      (append
       (hash-bind ImportMarkerKey (EMarker d-name))
       (strip-vec
        (foreach b env
...
(define (get-file-env filename all)
  ...
  (if filename
      (or (env-import (env-load filename) all [modname])
          *dummy-env*)))
